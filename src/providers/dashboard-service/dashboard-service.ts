...
export class DashboardServiceProvider {
  ...
  pollsApi:string = 'https://ssf-memedata.herokuapp.com/api/pollSets'+`${this.accessToken}`;
  memesApi:string = 'https://ssf-memedata.herokuapp.com/api/memes'+`${this.accessToken}`;
  categoryFilter:string = '?filter=%7B%22where%22%3A%7B%22pollCategory%22%3A%22'+`${this.selectedCategory}`+'%22%7D%7D';
  categoryApi:string = 'https://ssf-memedata.herokuapp.com/api/pollSets?filter=%7B%22where%22%3A%7B%22pollCategory%22%3A%22travel%22%7D%7D&access_token=b9mlT8uvLmKJj38eoquDnslnogB07V0mYpd4FDhAhRfT9twx9uf5REChqXEkMK2I';

  catFilterA:string = 'https://ssf-memedata.herokuapp.com/api/pollSets?filter=%7B%22where%22%3A%7B%22pollCategory%22%3A%22';
